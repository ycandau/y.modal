...
#define BANK_CNT_DEF	10		// Default number of banks, if argument is not provided
#define RESON_MAX_DEF	150		// Default maximum number of modes, if argument is not provided
#define STATE_CNT_DEF 10		// Default number of states, if argument is not provided
...
{
	MODE_FIX_CHG,		// Fixed modes: used in all_on, all_of
	MODE_FIX_ON,		// and individual resonator control: resonator on / off / toggle
	MODE_FIX_DOWN,
	MODE_FIX_OFF,
	MODE_CYC_WAIT,
...
} t_mode_ind;	// Has to be before because MODE_LAST is used to define a static array in t_modal

typedef enum _mode_type
{
	MODE_TYPE_OFF,			// The resonator is off: no processing in the perform function
	MODE_TYPE_FIX,			// The resonator is fixed: no ramping
	MODE_TYPE_VAR_A,		// The resonator is variable: amplitude ramping
	MODE_TYPE_VAR_P,		// The resonator is variable: parameter ramping
	MODE_TYPE_VAR_AP,		// The resonator is variable: amplitude and parameter ramping
	MODE_TYPE_VAR_S			// The resonator is variable: pitch shifting
...
{
	// XXX MODE_CNTD_INDEF,		// No countdown, indefinite time
	MODE_CNTD_RESON,		// Get countdown values from resonator
	MODE_CNTD_BANK,			// Get countdown values from bank
	// XXX MODE_CNTD_MODE			// Get countdown values from mode
...
typedef struct _resonator
{
	...
	t_double in_U_cur;		// For input amplitude modulation: current abscissa value: 0 to 1
	t_double in_A_cur;		// Current ordinate value: amplitude, 0 to 1
	t_double in_U_targ;		// For input amplitude modulation: target abscissa value: 0 to 1
	...
	t_int32			diff_sto;			// Store the diffusion channel index, or the number of channels, until set: diff set / rand / randr
	t_bool			diff_chg;			// Indicate that the diffusion channels have changed
