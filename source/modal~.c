/**
 @file
 modal~ - A bank of two pole resonators for modal synthesis
...
static t_symbol *sym_ampl;
static t_symbol *sym_freq;
static t_symbol *sym_decay;
...
void *modal_new(t_symbol *sym, t_int32 argc, t_atom *argv)
{
	...
	x->obj.z_misc |= Z_NO_INPLACE;			// Separate arrays for in and out
	...
	// If one argument is provided, it is assumed to be the number of banks
	else if ((argc == 1) && (atom_gettype(argv) == A_LONG) && (atom_getlong(argv) >= 1)) {

		x->bank_cnt	 = (t_int32)atom_getlong(argv);
		x->reson_max = RESON_MAX_DEF;
		state_cnt = STATE_CNT_DEF;	}

	// If two arguments are provided, they are the number of banks and the maximum number of resonators
	else if ((argc == 2)
	...
	// If three arguments are provided, they are the number of banks, the maximum number of resonators
	...
	if (!x->bank_arr) { MY_ERR("modal_new:  Failed to allocate bank_arr."); return NULL; }

	// Constructors for the banks
	for (int i = 0; i < x->bank_cnt; i++) {
		if (bank_new(x, x->bank_arr + i, 1) == ERR_ALLOC) { return NULL; } }
	...
	if (!x->state_arr) { MY_ERR("modal_new:  Failed to allocate state_arr."); return NULL; }
	...
	if ((!x->state_tmp->U_arr) || (!x->state_tmp->A_arr)) {
		MY_ERR("modal_new:  Failed to allocate state_tmp."); return NULL; }
	...
	if (!x->outp_mess_arr) { MY_ERR("modal_new:  Failed to allocate mess_arr."); return NULL; }
...
{
	//TRACE("modal_perform64");
	//POST("Clipping: STA_RAMP: Res %i, y = %f, i = %i, chunk = %i, counter = %i, cntd = %i", reson - bank->reson_arr, tmp, i, chunk_len, counter, reson->cntd);

	// Input and output vectors
	t_double *in = ins[0];
	t_double *out0 = outs[0];	t_double *out1 = outs[1];	t_double *out2 = outs[2];	t_double *out3 = outs[3];
	t_double *out4 = outs[4];	t_double *out5 = outs[5];	t_double *out6 = outs[6];	t_double *out7 = outs[7];

	// Set all output vectors to 0
	for (int i = 0; i < sampleframes; i++) {
		out0[i] = 0; out1[i] = 0; out2[i] = 0; out3[i] = 0; out4[i] = 0; out5[i] = 0; out6[i] = 0; out7[i] = 0; }
...
t_bank *bank_find(t_modal *x, t_atom *argv, t_symbol *sym)
{
	...
	if (atom_gettype(argv) == A_LONG) {
		...
		if ((index >= 0) && (index < x->bank_cnt)) { return (x->bank_arr + index); } }
	...
	// If the atom contains a symbol
		...
		for (int i = 0; i < x->bank_cnt; i++) {
			if ((x->bank_arr + i)->name == sym) { return (x->bank_arr + i); } } }
